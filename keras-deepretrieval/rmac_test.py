...
class config:
    def __init__(self):
        self.temp_dir = 'tmp'
        self.weights = './class_tmp/VGG16_ox epoch:45 Fri May 25 10:02:40 2018.h5'
        self.S = 800
        self.L = 3
        self.dataset = 'datasets/Oxford'
        self.dataset_name = 'Oxford'
        self.eval_binary = 'datasets/evaluation/compute_ap'
        self.num_rois = 14
        self.multires = False
        self.means = np.array([103.93900299,  116.77899933,  123.68000031], dtype=np.float32)[None, None, :]

class ImageHelper:
    def __init__(self, S, L, num_rois):
        self.S = S
        self.L = L
        self.num_rois = num_rois

    def load_and_prepare_image(self, fname, means, roi):
        ...
        im_resized = im_resized - means
        roi = np.round(roi * ratio).astype(np.int32)
        roi[2] = roi[2] - roi[0]
        roi[3] = roi[3] - roi[1]
        roi = [np.float32(x) for x in roi]
        return im_resized, roi
    ...
    def get_rmac_region_coordinates(self, H, W, L):
        ...
        return np.array(regions_xywh).astype(np.float32) # return xywh

    def prepare_image_and_grid_regions_for_network(self, fname, means, roi=None):
        # Extract image, resize at desired size, and extract roi region if
        # available. Then compute the rmac grid in the net format: X Y W H
        im_resized,roi = self.load_and_prepare_image(fname, means,roi)
        ...
        Region = self.pack_regions_for_network(all_regions)
        if roi is not None:
            Region = np.append(Region,np.array(roi))
            ## ATTENTION !!!
            Region = Region.reshape(21,4)
        return im_resized, Region

    def get_rmac_features(self, I, R, weights):
        image = Input(shape=(None, None, 3))
        roi = Input(shape=(21, 4))

        embedding = Rmac_net(image, roi, self.num_rois)
        model = Model(inputs=[image, roi],outputs=embedding)
        model.load_weights(weights,by_name=True)
        model.summary()
        #plot_model(model,  ='./tmp/rmac.png', show_shapes=True)
        # Compute RMAC vector
        I = np.expand_dims(I,axis=0)
        R = np.expand_dims(R,axis=0)
        print('Extracting RMAC from image...')
        RMAC = model.predict([I, R])
...
class Dataset:
    ...
    def score(self, sim, temp_dir, eval_bin):
        ...
        idx = np.initialort(sim, axis=1)[:, ::-1]
...
def extract_features(dataset, image_helper, weights, means):
    Ss = [C.S, ] if not C.multires else [C.S - 250, C.S, C.S + 250]
    # First part, queries
    for S in Ss:
        # Set the scale of the image helper
        image_helper.S = S
        out_queries_fname = "{0}/{1}_S{2}_L{3}_queries.npy".format(C.temp_dir, C.dataset_name, S, C.L)
        if not os.path.exists(out_queries_fname):
            ...
            for i in tqdm(range(N_queries), file=sys.stdout, leave=False, dynamic_ncols=True):
                ...
                I, R = image_helper.prepare_image_and_grid_regions_for_network(dataset.get_query_filename(i), means=means, roi=dataset.get_query_roi(i))
                features_queries[i] = image_helper.get_rmac_features(I, R, weights)
            np.save(out_queries_fname, features_queries)
    features_queries = np.dstack([np.load("{0}/{1}_S{2}_L{3}_queries.npy".format(C.temp_dir, C.dataset_name, S, C.L)) for S in Ss]).sum(axis=2)
    features_queries /= np.sqrt((features_queries * features_queries).sum(axis=1))[:, None]

    # Second part, dataset
    for S in Ss:
        image_helper.S = S
        out_dataset_fname = "{0}/{1}_S{2}_L{3}_dataset.npy".format(C.temp_dir, C.dataset_name, S, C.L)
        if not os.path.exists(out_dataset_fname):
            ...
            for i in tqdm(range(N_dataset), file=sys.stdout, leave=False, dynamic_ncols=True):
                ...
                features_dataset[i] = image_helper.get_rmac_features(I, R, weights)
            np.save(out_dataset_fname, features_dataset)
    features_dataset = np.dstack([np.load("{0}/{1}_S{2}_L{3}_dataset.npy".format(C.temp_dir, C.dataset_name, S, C.L)) for S in Ss]).sum(axis=2)
...
dataset = Dataset(C.dataset, C.eval_binary)
image_helper = ImageHelper(C.S, C.L, C.num_rois)

 # Extract features
features_queries, features_dataset = extract_features(dataset, image_helper, C.weights, C.means)

# Database side expansion?
if C.dbe is not None and C.dbe > 0:
    ...
    idx = np.initialort(X, axis=1)[:, ::-1]
    weights = np.hstack(([1], (C.dbe - np.arange(0, C.dbe)) / float(C.dbe)))
    weights_sum = weights.sum()
    features_dataset = np.vstack(
        [np.dot(weights, features_dataset[idx[i, :C.dbe + 1], :]) / weights_sum for i in
            range(len(features_dataset))])

    # Compute similarity
sim = features_queries.dot(features_dataset.T)
    # Average query expansion?
if C.aqe is not None and C.aqe > 0:
    ...
    features_queries = np.vstack(
        [np.vstack((features_queries[i], features_dataset[idx[i, :C.aqe]])).mean(axis=0) for i in
            range(len(features_queries))])
        # for i in range(features_queries.shape[0]):
        #    features_queries[i] = np.vstack((features_queries[i], features_dataset[idx[i, :C.aqe]])).mean(axis=0)
    sim = features_queries.dot(features_dataset.T)

    # Score
dataset.score(sim, C.temp_dir, C.eval_binary)
