...
###   no v_flip \ no brightness  \ no rotation \ crop and padding 101 \ my_metirc_iou_2   \ thresholds > 0.0  \ TTA hflip
...
def augmentation(x_trian, y_train):
    for i in range(len(x_trian)):
        input_image, output_image = random_crop(x_trian[i], y_train[i], 85, 85)
        input_image = cv2.copyMakeBorder(input_image, 8, 8, 8, 8, cv2.BORDER_REFLECT)
        output_image = cv2.copyMakeBorder(output_image, 8, 8, 8, 8, cv2.BORDER_REFLECT)
...
x_test = np.array([(np.array(load_img("./data/images/{}.png".format(idx), grayscale = True))) / 255 for idx in tqdm_notebook(test_df.index)]).reshape(-1, img_size_target, img_size_target, 1)
