...
parser.add_argument('--padding', type=str2bool, default=True, help='Whether to randomly padding the image for data augmentation')
...
def augmentation(x_trian, y_train):
    for i in range(len(x_trian)):
        if args.padding:
            input_image, output_image = random_crop(x_trian[i], y_train[i], 93, 93)
            input_image = cv2.copyMakeBorder(input_image, 4, 4, 4, 4, cv2.BORDER_REFLECT)
            output_image = cv2.copyMakeBorder(output_image, 4, 4, 4, 4, cv2.BORDER_REFLECT)
