...
def transform(imgs, masks, config):
    ...
    for id in range(len(imgs)):
        ...
        if config.do_random_shift_scale_crop_pad and np.random.randint(2):
...
CONFIG = AttrDict({'doresize': True, 'do_flip_transpose': False,
...
model1.load_weights('xception.h5', by_name=True)
...
history = model.fit(x_train, y_train,
                    ...
                    epochs=epochs,
                    batch_size=batch_size,
                    callbacks=[ model_checkpoint,learning_rate,early_stopping, TensorBoard(log_dir='mytensorboard/lovasz')],
                    verbose=2)
