...
def do_center_pad_to_factor2(data, factor=32):
    image, mask = data[:,:,0], data[:,:,1]
...
def do_random_pad_to_factor2(data, limit=(-4,4), factor=32):
    image, mask = data[:,:,0], data[:,:,1]
    H,W = image.shape[:2]
...
def do_brightness_shift(data, alpha=0.125):
    image, mask = data[:,:,0], data[:,:,1]
    image = image + alpha
...
def do_brightness_multiply(data, alpha=1):
    image, mask = data[:,:,0], data[:,:,1]
    image = alpha*image
    image = np.clip(image, 0, 1)
    return (image, mask)
...
def do_flip_transpose2(data, type=0):
...
def do_random_shift_scale_crop_pad2(data, limit=0.10):
    image, mask = data[:,:,0], data[:,:,1]
    H, W = image.shape[:2]
...
def do_shift_scale_rotate2(data, dx=0, dy=0, scale=1, angle=0 ):
    image, mask = data[:,:,0], data[:,:,1]
...
def do_elastic_transform2(data, grid=32, distort=0.2):
    image, mask = data[:,:,0], data[:,:,1]
    borderMode=cv2.BORDER_REFLECT_101
    height, width = image.shape[:2]
...
def do_horizontal_shear2(data, dx=0 ):
    image, mask = data[:,:,0], data[:,:,1]
    borderMode=cv2.BORDER_REFLECT_101
    #cv2.BORDER_REFLECT_101  cv2.BORDER_CONSTANT
