...
# valid_data = np.stack((x_valid, y_valid), axis=1).reshape(-1, 2, 101, 101).transpose(0, 2, 3, 1)
# valid_data = np.array([resize_transform(x) for x in valid_data]).reshape(-1, 2, img_size_target, img_size_target).transpose(0, 2, 3, 1)
# x_valid, y_valid = valid_data[:,:,:,0].reshape(-1, img_size_target, img_size_target, 1), valid_data[:,:,:,1].reshape(-1, img_size_target, img_size_target, 1)


# train_data = np.stack((x_train, y_train), axis=1).reshape(-1, 2, 101, 101).transpose(0, 2, 3, 1)
# train_data = np.array([resize_transform(x) for x in train_data]).reshape(-1, 2, img_size_target, img_size_target).transpose(0, 2, 3, 1)
# x_train, y_train = train_data[:,:,:,0].reshape(-1, img_size_target, img_size_target, 1), valid_data[:,:,:,1].reshape(-1, img_size_target, img_size_target, 1)
